...
func LoadConfig(ctx context.Context) (Config, error) {
	...
	return loadConfigByKey(ctx, "storage_config", DefaultConfig())
}
...
func loadConfigByKey(ctx context.Context, key string, fallback Config) (Config, error) {
	...
	if err := json.Unmarshal([]byte(val), &fallback); err != nil {
...
func SaveActiveConfig(ctx context.Context, cfg Config) error {
	return saveSystemConfig(ctx, "storage_config", cfg, "文件存储驱动及连接配置（JSON）")
}
...
func upsertSystemConfig(_ context.Context, tx *gorm.DB, key string, value any, description string) error {
