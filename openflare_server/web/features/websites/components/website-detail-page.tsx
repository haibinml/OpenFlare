    ...
    <>
                        ...
                        <td className="px-3 py-4">
                          <div className="flex flex-wrap gap-2">
                            <StatusBadge
                              ...
                              variant={
                                route.enable_https ? 'success' : 'warning'
                              }
                            />
                            {route.redirect_http ? (
      ...
      {isCertificateEditorOpen ? (
        <CertificateEditorModal
          ...
          onConvert={(manualCertificate) => {
            setIsCertificateEditorOpen(false);
      ...
      {convertCertificate ? (
        <CertificateApplyModal
          isOpen={true}
          onClose={() => setConvertCertificate(null)}
          mode="convert-upload"
          certificate={convertCertificate}
          onApplied={(convertedCertificate) => {
            setFeedback({
              tone: 'success',
              message: `证书 ${convertedCertificate.name} 转换申请已提交。`,
            });
          }}
        />
      ) : null}
